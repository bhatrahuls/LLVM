...
class LLVMBackend(Backend):
    ...
    def _create_alloca(self, name, typ):
        ...
        with self.builder.goto_entry_block():
            allocation = self.builder.alloca(typ, size=None, name=name)
        return allocation
    ...
    def VarDef(self, node: VarDef):
        ...
        temp = self.visit(node.var)
        allocation = self._create_alloca(temp['name'], temp['type'])
        val = self.visit(node.value)
        self.builder.store(val, allocation)
        self.func_symtab[-1][temp['name']] = allocation
    ...
    def FuncDef(self, node: FuncDef):
        ...
        for arg in func.args:
            allocation = self._create_alloca(arg.name, arg.type)
            self.builder.store(arg, allocation)
            self.func_symtab[-1][arg.name] = allocation
